...
let livingNames = living.map((s) => s.name);
console.log("Living scripts: ", livingNames);

let ttb = SCRIPTS.filter((s) => s.direction === "ttb");
let ttbNames = ttb.map((s) => s.name);
...
let livingYears = SCRIPTS.filter((s) => s.living).map((s) => s.year);
let deadYears = SCRIPTS.filter((s) => !s.living).map((s) => s.year);
...
let avgLivingYear = Math.round(total / count);
...
let avgDeadYear = Math.round(total / count);
...
function textScripts(txt) {
  let scripts = countBy(txt, (char) => {
    let s = characterScript(char.codePointAt(0));
