...
const livingNames = living.map((s) => s.name);
console.log('Living scripts: ', livingNames);

const ttb = SCRIPTS.filter((s) => s.direction === 'ttb');
const ttbNames = ttb.map((s) => s.name);
console.log('Top-to-bottom scripts: ', ttbNames);

function characterCount(s) {
  return s.ranges.reduce((count, [range_i, range_f]) => {
    return count + (range_f - range_i);
  }, 0);
}
...
for (const s of SCRIPTS) {
  if (smallest === null || characterCount(smallest) > characterCount(s)) {
    smallest = s;
  }
}

console.log('Least characters:', smallest);

function average(arr) {
  return arr.reduce((a, b) => a + b) / arr.length;
}

const livingYears = SCRIPTS.filter((s) => s.living).map((s) => s.year);
const deadYears = SCRIPTS.filter((s) => !s.living).map((s) => s.year);
console.log('Average living year:', Math.round(average(livingYears)));
console.log('Average dead year:', Math.round(average(deadYears)));
...
const avgLivingYear = Math.round(total / count);
console.log('Average living year:', avgLivingYear);
...
const avgDeadYear = Math.round(total / count);
console.log('Average dead year:', avgDeadYear);
...
function textScripts(txt) {
  const scripts = countBy(txt, (char) => {
    const s = characterScript(char.codePointAt(0));
